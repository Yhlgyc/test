...
class Paddle {
  constructor (_main) {
    let p = {
      ...
      w: _main.paddle_w,                                   // 图片宽度
      h: _main.paddle_h,                                   // 图片高度
      speed: 80,                                           // x轴移动速度
...
class Scene {
  ...
  creatBlockList (game) {
    let lv = this.lv,                         // 游戏难度级别
        c_w = game.canvas.width,              // canvas宽度
        c_h = game.canvas.height,             // canvas高度
        xNum_max = c_w/50,                    // x轴砖块最大数量
        yNum_max = 12,                        // y轴砖块最大数量
        x_start = 0,                          // x轴起始坐标，根据砖块数量浮动
        y_start = game.canvas.height * 0.35    // y轴起始坐标，默认从60起
